...
function addCons($dbConn){
	...
	$sql = "INSERT INTO tbl_courier (cons_no, ship_name, phone, s_add, rev_name, r_phone, r_add,  type, weight, invice_no, qty, book_mode, freight, mode, pick_date, pick_time, status, comments, book_date, soffice, current_city )
			VALUES('$ConsignmentNo', '$Shippername','$Shipperphone', '$Shipperaddress', '$Receivername','$Receiverphone','$Receiveraddress', '$Shiptype',$Weight , '$Invoiceno', $Qnty, '$Bookingmode', '$Totalfreight', '$Mode', '$Packupdate', '$Pickuptime', '$status', '$Comments', NOW(), '$Soffice','$Coffice' )";	
	//echo $sql;
	dbQuery($dbConn,$sql);
...
function updateStatus($dbConn) {
	...
	$sql = "UPDATE tbl_courier 
				SET status = '$status', current_city ='$Coffice'
				WHERE cid = $cid
				AND cons_no = '$cons_no'";
	dbQuery($dbConn,$sql);	
